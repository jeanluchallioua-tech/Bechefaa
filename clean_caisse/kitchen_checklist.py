...
def register_kitchen_checklist(app, db, ensure_order_schema, order_payload):
    ...
    def finish_kitchen_order(order_id):
        """Marque la commande terminée sans la supprimer ni la verrouiller pour l'historique."""
        now = int(time.time() * 1000)
        try:
            with db() as conn:
                with conn.transaction():
                    ...
                    if order["status"] not in ("À préparer", "En préparation"):
                        return jsonify({"ok": False, "error": "Cette commande ne peut pas être terminée", "status": order["status"]}), 409
                    remaining = conn.execute(
                        "SELECT COUNT(*) AS n FROM caisse_order_items WHERE order_id=%s AND prepared=FALSE",
                        (order_id,),
                    ).fetchone()["n"]
                    if int(remaining) > 0:
                        return jsonify({"ok": False, "error": "Tous les plats doivent être cochés avant de terminer", "remaining": int(remaining)}), 409
                    conn.execute(
            ...
            return jsonify({"ok": False, "error": "Finalisation cuisine impossible", "detail": str(exc)}), 500
...
function card(o){let finished=o.status==='Terminée';let items=(o.items||[]).map(i=>`<label class="item ${i.prepared?'done':''}"><input type="checkbox" ${i.prepared?'checked':''} ${finished?'disabled':''} data-order="${esc(o.id)}" data-line="${esc(i.line_id)}"><div class="itemtext">${esc(i.qty)}× ${esc(i.name)}${i.options_text?`<div class="opts">${esc(i.options_text)}</div>`:''}</div></label>`).join('');let allDone=(o.items||[]).length>0&&(o.items||[]).every(i=>i.prepared);let button=!finished?`<button class="finish" data-finish="${esc(o.id)}" ${allDone?'':'disabled'}>Terminer</button>`:'';return `<article class="card ${o.status==='En préparation'?'preparing':''} ${finished?'finished':''}"><div class="cardhead"><div><div class="num">#${esc(o.num)} · ${esc(o.customer_name)}</div><div class="hint">${new Date(Number(o.created_at)).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}</div></div><span class="tag">${esc(o.ticket_type)}</span></div>${items||'<div class="empty">Aucun article</div>'}${button}</article>`}
