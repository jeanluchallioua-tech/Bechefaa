...
function optionRows(i){let opts=Array.isArray(i.options)?i.options:[];let rows=opts.map(o=>{if(o&&typeof o==='object'){let g=String(o.group||'').trim(),n=String(o.name||o.label||'').trim();if(!n)return '';return `<div class="optrow">${g?`<span class="optgroup">${esc(g)} :</span> `:''}${esc(n)}</div>`}return o!=null?`<div class="optrow">${esc(o)}</div>`:''}).filter(Boolean);if(rows.length)return `<div class="opts">${rows.join('')}</div>`;return i.options_text?`<div class="opts"><div class="optrow">${esc(i.options_text)}</div></div>`:''}
