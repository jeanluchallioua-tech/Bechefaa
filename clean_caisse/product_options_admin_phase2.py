...
def register_product_options_admin_phase2(app, db):
    ...
    def options_admin_page():
        return Response(r'''<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>BÉCHÉFAA • Options</title><style>*{box-sizing:border-box}body{margin:0;font-family:Arial;background:#f4f5f7;color:#17191c}.top{background:#111827;color:white;padding:14px 22px}.top a{color:white;margin-left:18px}.wrap{max-width:1150px;margin:auto;padding:24px}.card,.group{background:white;border-radius:14px;padding:18px;margin:14px 0}.group{border:1px solid #d9dde3}.opts{display:grid;grid-template-columns:repeat(auto-fill,minmax(340px,1fr));gap:8px;margin-top:12px}.opt{border:1px solid #e4e7ec;border-radius:9px;padding:9px;display:grid;grid-template-columns:minmax(120px,1fr) 62px 82px 62px;gap:7px;align-items:center}.opt input{width:100%;padding:8px;border:1px solid #ccd1d8;border-radius:7px}.opt button{border:0;border-radius:7px;padding:9px;background:#14804a;color:white;font-weight:700;cursor:pointer}.opt .namebtn{background:#344054}.key,.hint{color:#667085;font-size:13px}.good{background:#e8f7ee;padding:10px;border-radius:8px}.bad{background:#fff0ee;color:#9d261d;padding:10px;border-radius:8px}select{width:100%;padding:11px}.action{display:inline-block;margin-top:8px;background:#175cd3;color:#fff;text-decoration:none;padding:11px 16px;border-radius:8px;font-weight:700}</style></head><body><div class="top"><b>BÉCHÉFAA • Administration</b><a href="/administration/produits">Produits</a><a href="/pos">Caisse</a></div><div class="wrap"><h1>Options et suppléments</h1><p class="hint">Modification sécurisée du nom et du prix. Aucun index existant n'est déplacé.</p><div id="status">Chargement…</div><div class="card"><h2>Ajouter ou affecter une option</h2><p class="hint">La fonction validée de Phase 2.3 est maintenant accessible depuis l'administration. Elle ajoute uniquement en fin de liste et conserve les indices existants.</p><a class="action" href="/administration/options-ajout-test">Ajouter / affecter une option</a></div><div class="card"><h2>Groupes existants</h2><div id="groups"></div></div><div class="card"><h2>Affectation par produit</h2><select id="product"></select><div id="assignment"></div></div></div><script>
