        ...
        try:
            with db() as conn:
                with conn.transaction():
                    ...
                    sent_to_kitchen = old_status in ("À préparer", "En préparation", "Prête", "Terminée")
...
function editOrder(id){let o=ORDERS.find(x=>x.id===id);if(!o)return;EDIT=JSON.parse(JSON.stringify(o));document.getElementById('mtitle').textContent='Modifier commande #'+o.num;document.getElementById('mmsg').innerHTML=o.status==='Terminée'?'<div class="notice">Commande terminée : toute modification la renverra en cuisine avec le statut À préparer.</div>':(o.status==='À préparer'||o.status==='En préparation'||o.status==='Prête'?'<div class="notice">Cette commande a déjà été envoyée en cuisine. La modification la replacera dans À préparer.</div>':'');document.getElementById('modal').classList.add('open');renderEdit()}
