...
def register_option_add_test_phase23(app, db):
    ...
    def option_add_test_page():
        ...
        html = f'''<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>BÉCHÉFAA • Options</title><style>
*{{box-sizing:border-box}}body{{font-family:Arial;background:#fff;margin:0;color:#17191c}}.wrap{{max-width:920px;margin:auto;padding:14px}}.grid{{display:grid;grid-template-columns:1fr 1fr;gap:14px}}.card{{background:#fafafa;border:1px solid #e4e7ec;padding:16px;border-radius:12px}}h2{{margin:0 0 4px;font-size:18px}}label{{display:block;font-weight:700;margin-top:12px;font-size:13px}}select,input{{width:100%;padding:10px;margin-top:5px;border:1px solid #ccd1d8;border-radius:8px;background:#fff;font-size:14px}}input::placeholder{{color:#98a2b3}}button{{margin-top:14px;background:#111;color:#fff;border:0;padding:10px 14px;border-radius:8px;font-weight:800;cursor:pointer}}button.danger{{background:#b42318;margin-left:6px}}.ok{{background:#e8f7ee;padding:10px;border-radius:8px;margin-bottom:12px}}.err{{background:#fff0ee;color:#9d261d;padding:10px;border-radius:8px;margin-bottom:12px}}.hint{{color:#667085;font-size:12px;line-height:1.4}}@media(max-width:700px){{.grid{{grid-template-columns:1fr}}}}
</style></head><body><div class="wrap">{message_html}{error_html}<div class="grid"><div class="card"><h2>Ajouter une option</h2><p class="hint">Choisissez le groupe, saisissez le nom et le prix.</p><form method="post"><input type="hidden" name="action" value="add"><label>Groupe</label><select name="group_key" required>{options_html}</select><label>Nom de l’option</label><input name="name" maxlength="120" placeholder="Ex. Bacon, Sauce maison, Avocat…" required><label>Prix supplémentaire</label><input name="price" type="number" min="0" step="0.01" placeholder="Ex. 2,00" value="0.00" required><button type="submit">Ajouter l’option</button></form></div><div class="card"><h2>Affecter à un produit</h2><p class="hint">Choisissez le produit et l’option à ajouter ou retirer.</p><form method="post" onsubmit="var v=this.option_choice.value.split('|');this.assign_group_key.value=v[0];this.option_index.value=v[1]"><input type="hidden" name="assign_group_key"><input type="hidden" name="option_index"><label>Produit</label><select name="product_id" required>{products_html}</select><label>Option</label><select name="option_choice" required>{assign_options_html}</select><button type="submit" name="action" value="assign">Affecter</button><button class="danger" type="submit" name="action" value="unassign" onclick="return confirm('Retirer cette option uniquement de ce produit ?')">Retirer</button></form></div></div></div></body></html>'''
