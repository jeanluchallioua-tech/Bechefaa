...
def _clean_customer(raw):
    ...
    return {"first_name":first_name,"last_name":last_name,"display_name":display_name,"phone":phone,"email":email,"address":address,"postal_code":postal_code,"city":city,"door_intercom":door_intercom}


def _has_customer(c):
    return any(c.get(k) for k in ("first_name","last_name","phone","email","address","postal_code","city","door_intercom"))
...
def _save_customer(conn, customer):
    ...
    if existing:
        conn.execute("""UPDATE caisse_clients SET first_name=%s,last_name=%s,display_name=%s,phone=%s,email=%s,address=%s,postal_code=%s,city=%s,door_intercom=%s,updated_at=%s WHERE id=%s""",
            (customer["first_name"],customer["last_name"],display_name,customer["phone"],customer["email"],customer["address"],customer["postal_code"],customer["city"],customer["door_intercom"],now,client_id))
    else:
        conn.execute("""INSERT INTO caisse_clients (id,first_name,last_name,display_name,phone,email,address,postal_code,city,door_intercom,notes,created_at,updated_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'',%s,%s)""",
            (client_id,customer["first_name"],customer["last_name"],display_name,customer["phone"],customer["email"],customer["address"],customer["postal_code"],customer["city"],customer["door_intercom"],now,now))
