...
function optionText(v){if(v===null||v===undefined)return'';if(typeof v==='string'||typeof v==='number')return esc(v);if(typeof v==='object'){let n=v.name||v.label||v.title||v.value||v.id||'';let price=Number(v.price||v.extraPrice||v.supplement||0);return esc(n)+(price?' (+'+price.toFixed(2).replace('.',',')+' €)':'')}return esc(String(v))}
