...
def caisse_favicon():
    source = Image.open(io.BytesIO(base64.b64decode(CAISSE_ICON_B64))).convert("RGBA")
    output = io.BytesIO()
    source.save(output, format="ICO", sizes=[(16,16),(32,32),(48,48),(64,64),(128,128),(256,256)])
    response = Response(output.getvalue(), mimetype="image/x-icon")
    response.headers["Cache-Control"] = "no-cache, no-store, must-revalidate"
    return response
