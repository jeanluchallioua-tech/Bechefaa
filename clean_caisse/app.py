...
def send_order_to_kitchen(order_id):
    ...
    try:
        with db() as conn:
            with conn.transaction():
                row = conn.execute(
                    "SELECT id, num, status, total FROM caisse_orders WHERE id = %s FOR UPDATE",
                    (order_id,),
                ).fetchone()
                if not row:
                    return jsonify({"ok": False, "error": "Commande introuvable"}), 404
                if row["status"] == "À préparer":
                    return jsonify({"ok": True, "id": row["id"], "num": row["num"], "total": float(row["total"]), "status": "À préparer"}), 200
                if row["status"] != "Enregistrée":
                    return jsonify({"ok": False, "error": "Cette commande ne peut pas être envoyée en cuisine", "status": row["status"]}), 409
                conn.execute(
    ...
    return jsonify({"ok": True, "id": order_id, "num": row["num"], "total": float(row["total"]), "status": "À préparer"}), 200
...
</style></head><body><div class="top"><b>BÉCHÉFAA-Caisse</b><span>Nouvelle caisse PostgreSQL</span><span class="status" id="status">Chargement…</span></div><div class="layout"><aside class="cats" id="cats"></aside><main class="main"><div class="title"><h2 id="title">Catalogue</h2><span id="count"></span></div><div class="grid" id="grid"></div></main><aside class="cart"><h2 id="side-title">Options produit</h2><div id="options"><div class="empty">Cliquez sur un produit pour sélectionner ses options.</div></div><div class="order-box"><h2>Commande</h2><div id="order-message"></div><div id="order"><div class="empty">Commande vide</div></div></div><div class="note">Étape 3 : une commande enregistrée peut maintenant être envoyée en cuisine. Le statut PostgreSQL passe de <b>Enregistrée</b> à <b>À préparer</b>.</div></aside></div>
<script>
let DATA=null,current=null,currentProduct=null,currentGroups=[],selections={},ORDER=[],LAST_SAVED_ORDER=null;const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot',"'":'&#39;'}[c]));
