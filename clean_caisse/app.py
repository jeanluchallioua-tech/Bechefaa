...
*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;background:#f4f5f7;color:#17191c}.top{height:64px;background:#111827;color:white;display:flex;align-items:center;padding:0 22px;gap:20px}.top b{font-size:22px}.status{margin-left:auto;font-size:13px}.layout{display:grid;grid-template-columns:190px 1fr 340px;height:calc(100vh - 64px)}.cats{background:#fff;border-right:1px solid #ddd;padding:12px;overflow:auto}.cat{width:100%;padding:13px 10px;margin:4px 0;border:0;border-radius:8px;background:#f0f1f3;text-align:left;font-weight:700;cursor:pointer}.cat.active{background:#111827;color:#fff}.main{padding:18px;overflow:auto}.title{display:flex;justify-content:space-between;align-items:center;margin-bottom:14px}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(170px,1fr));gap:12px}.product{background:#fff;border:1px solid #ddd;border-radius:12px;padding:14px;min-height:118px;cursor:pointer;display:flex;flex-direction:column}.product:hover{box-shadow:0 2px 10px #0002}.product-photo{width:100%;height:120px;object-fit:cover;border-radius:8px;margin-bottom:10px}.name{font-weight:800;font-size:16px}.meta{font-size:12px;color:#666;margin-top:7px}.price{font-size:20px;font-weight:800;margin-top:auto;padding-top:10px}.cart{background:#fff;border-left:1px solid #ddd;padding:18px;overflow:auto}.cart h2{margin-top:0}.empty{color:#777;padding:30px 0;text-align:center}.badge{font-size:11px;background:#e8eefc;border-radius:20px;padding:4px 8px;margin-top:8px;display:inline-block;width:max-content}.note{font-size:12px;color:#666;margin-top:20px;border-top:1px solid #eee;padding-top:12px}.opt-group{border-top:1px solid #eee;padding-top:12px;margin-top:12px}.opt-head{display:flex;justify-content:space-between;gap:8px;align-items:center;margin-bottom:7px}.opt-head b{display:block}.rule{font-size:11px;color:#777}.opt-value{width:100%;font-size:13px;background:#f4f5f7;border:1px solid #e3e5e8;border-radius:8px;padding:9px;margin:5px 0;cursor:pointer;text-align:left;display:flex;justify-content:space-between;gap:8px}.opt-value:hover{background:#eceff3}.opt-value.selected{background:#111827;color:#fff;border-color:#111827}.opt-value .check{font-weight:800}.selection-summary{background:#f7f8fa;border:1px solid #e5e7eb;border-radius:10px;padding:10px;margin-bottom:10px;font-size:13px}.selection-summary b{display:block;margin-bottom:5px}.selection-price{font-size:18px;font-weight:800;margin-top:8px}.warning{font-size:12px;color:#a34a00;margin:8px 0}@media(max-width:900px){.layout{grid-template-columns:150px 1fr}.cart{display:none}}
</style></head><body><div class="top"><b>BÉCHÉFAA-Caisse</b><span>Nouvelle caisse PostgreSQL</span><span class="status" id="status">Chargement…</span></div><div class="layout"><aside class="cats" id="cats"></aside><main class="main"><div class="title"><h2 id="title">Catalogue</h2><span id="count"></span></div><div class="grid" id="grid"></div></main><aside class="cart"><h2 id="side-title">Options produit</h2><div id="options"><div class="empty">Cliquez sur un produit pour sélectionner ses options.<br>Aucun ajout au panier.</div></div><div class="note">Sélection locale uniquement dans le navigateur.<br>Aucune écriture PostgreSQL, aucune commande créée.</div></aside></div>
<script>
let DATA=null,current=null,currentProduct=null,currentGroups=[],selections={};const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
...
function renderOptions(){if(!currentProduct)return;let base=Number(currentProduct.price||0),extra=totalExtra();let picked=[];currentGroups.forEach((g,gi)=>{let info=groupInfo(g,gi);info.vals.forEach((v,vi)=>{if(selections[selectionKey(gi,vi)])picked.push(optionInfo(v).name)})});let summary=`<div class="selection-summary"><b>Sélection en cours</b>${picked.length?esc(picked.join(' • ')):'Aucune option sélectionnée'}<div class="selection-price">${(base+extra).toFixed(2).replace('.',',')} €</div></div>`;let groups=currentGroups.map((g,gi)=>{let info=groupInfo(g,gi);let rule=[];if(info.required)rule.push('obligatoire');if(info.max===1)rule.push('1 choix');else if(info.max>1)rule.push(info.max+' choix max');let vals=info.vals.map((v,vi)=>{let oi=optionInfo(v),key=selectionKey(gi,vi),sel=!!selections[key];return `<button class="opt-value ${sel?'selected':''}" data-gi="${gi}" data-vi="${vi}"><span>${sel?'✓ ':''}${esc(oi.name)}</span><span>${oi.price?('+'+oi.price.toFixed(2).replace('.',',')+' €'):''}</span></button>`}).join('');return `<div class="opt-group"><div class="opt-head"><b>${esc(info.name)}</b><span class="rule">${esc(rule.join(' • '))}</span></div>${vals||'<div class="empty">Aucune valeur</div>'}</div>`}).join('');document.getElementById('options').innerHTML=summary+groups}
...
fetch('/api/catalog/summary').then(r=>r.json()).then(d=>{DATA=d;document.getElementById('status').textContent=d.products+' produits • PostgreSQL';let cats=['',...d.categoryNames];document.getElementById('cats').innerHTML=cats.map(c=>`<button class="cat" data-cat="${esc(c)}">${esc(c||'Tous les produits')}</button>`).join('');document.getElementById('cats').onclick=e=>{let b=e.target.closest('.cat');if(!b)return;current=b.dataset.cat||null;render()};document.getElementById('grid').onclick=e=>{let p=e.target.closest('.product');if(!p)return;showOptions(p.dataset.id,p.dataset.name)};document.getElementById('options').onclick=e=>{let b=e.target.closest('.opt-value');if(!b)return;toggleOption(Number(b.dataset.gi),Number(b.dataset.vi))};render()}).catch(()=>{document.getElementById('status').textContent='Erreur catalogue';document.getElementById('grid').innerHTML='<p>Impossible de charger le catalogue.</p>'});
</script></body></html>'''
