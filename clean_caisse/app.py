...
def create_order():
        ...
        with db() as conn:
            with conn.transaction():
                ...
                conn.execute(
                    ...
                    (order_id, order_num, "Client comptoir", "CAISSE", "À ENCAISSER", "À préparer", total, now, now),
                )
    ...
    return jsonify({"ok": True, "id": order_id, "num": order_num, "total": float(total), "status": "À préparer"}), 201
